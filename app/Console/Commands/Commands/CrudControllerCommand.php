...
class CrudControllerCommand extends GeneratorCommand
{
    ...
    {
        return 'App\Modules\\'.$this->option('crud-name').'\Controllers';
    }
    ...
    protected function buildClass($name)
    {
        ...
        if($this->option('model-namespace') == ''){
          $modelNS = 'Modules\\'.$this->option('crud-name').'\Models\\';
        };
        ...
        $tableName = str_plural($crudName);
        $crudNameSingular = str_singular($crudName);
        $modelName = $this->option('model-name');
        $modelNamespace = $modelNS;
        $primaryKey = ($this->option('pk')) ? $this->option('pk') : 'id';
