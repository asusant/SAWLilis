...
class CrudCommand extends Command
{
    ...
    protected $signature = 'module:generate
                            ...
                            {--master= : Fields from a json file.}
    ...
    public function handle()
    {
        ...
        if ($this->option('master')) {
            $fields = $this->processJSONFields($this->option('master'));
        }
        ...
        if ($this->option('master')) {
            $foreignKeys = $this->processJSONForeignKeys($this->option('master'));
        }

        $validations = trim($this->option('validations'));
        if ($this->option('master')) {
            $validations = $this->processJSONValidations($this->option('master'));
        }
        ...
        $fillable = "['" . $commaSeparetedString . "', 'created_by', 'updated_by', 'deleted_by']";
        ...
        if ($this->option('master')) {
            $relationships = $this->processJSONRelationships($this->option('master'));
        }

        $formHelper = $this->option('form-helper');
        $softDeletes = $this->option('soft-deletes');

        $this->call('crud:controller', ['name' => $controllerNamespace . $name . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations, '--pk' => $primaryKey]);
        if($withApi == 'yes'){
            $this->call('crud:api-controller', ['name' => $controllerNamespace . $name . 'ApiController', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--pagination' => $perPage, '--validations' => $validations]);
        }
        ...
        $this->call('crud:view', ['name' => $name, '--fields' => $fields, '--validations' => $validations, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--localize' => $localize, '--pk' => $primaryKey, '--form-helper' => $formHelper, '--custom-data' => 'formHeadingForAjax' ]);
        $this->call('crud:route', ['name' => 'routes', '--module-name' => $name, '--with-api' => $withApi, '--route-group' => $routeGroup, '--crud-name' => $name]);
