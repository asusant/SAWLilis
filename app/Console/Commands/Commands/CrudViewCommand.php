...
class CrudViewCommand extends Command
{
    ...
    /**
     * Html headings for ajax (Yajra).
     *
     * @var string
     */
    protected $formHeadingForAjax = '';
     ...
     * @var integer
     */
    protected $defaultColumnsToShow = 4;
    ...
    public function handle()
    {
        ...
        if ($this->option('view-path')) {
            ...
            $path = 'App/Modules/'.$this->argument('name').'/Views/'.$this->viewName.'/';
        }
        ...
        foreach ($this->formFields as $key => $value) {
            ...
            $this->formHeadingForAjax .= "{ data: '".$field."' , name: '".$field."' },\n";
            $i++;
        }

        $this->templateStubs($path);
        // dd($this->formHeadingForAjax);
    ...
    protected function userDefinedVars($file)
    {
        $start = $this->delimiter[0];
        $end = $this->delimiter[1];
        $this->customData.='='.$this->formHeadingForAjax;
        // dd($this->customData);
